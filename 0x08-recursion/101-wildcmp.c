...
{
	/**
	 * this is going to be asum of return values
	 */
	int sum = 0;
	/**
	...
	if (*s1 == '\0' || *s2 == '\0')
		 ...
		 * if the chars are equal, add the return value
		 * of wildcmp() to sum
		 * add the return value of bandersnatch() to sum
		 *convert non-zero to 1, keeps 0 at 0, returnreturn (0);
		 */
		if (*s2 == '*')
		{
			s2 = move(s2);
 ...
 *
 * Return: the address of the character after *
 */
char *move(char *s2)
{
	/**
